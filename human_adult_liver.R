...
#   - Retain actually expressed genes
mt_ensembl_ids <- mt_genes$ensembl_gene_id
valid_mt_genes <- intersect(
  mt_ensembl_ids,
  rownames(counts)
  )
...
# Calculate QC metrics for each cell including mitochondrial gene percentages
qc_stats <- perCellQCMetrics(sce, subsets = list(Mito = valid_mt_genes))
# Automatically detect outlier thresholds using Median Absolute Deviation (MAD)
lib.thresholds <- isOutlier(qc_stats$sum, log=TRUE, type="lower") 
gene.thresholds <- isOutlier(qc_stats$detected, log=TRUE, type="lower")
mito.thresholds <- isOutlier(qc_stats$subsets_Mito_percent, type="higher")
# View automatically calculated thresholds
summary(lib.thresholds)  # Show filtering threshold for total UMIs
summary(gene.thresholds) # Show threshold for detected genes
summary(mito.thresholds) # Show threshold for mitochondrial percentage
...
# Save Plot 4
p4_path <- file.path(qc_path, "UMAP Batch Clustering.png")
...
#   - dir: Parent directory path
#   - mcmc_cd_path: Subdirectory for storing diagnostic plots
...
#   - red dashed lines at ±3 indicate significant divergence
p5 <- BASiCS_DiagPlot(
  chain,
...
mirtarbase <- read.csv(
  file.path(dir_mirtarbase, "hsa_MTI.csv"),
  stringsAsFactors = FALSE
  ) %>% 
  filter(miRNA %in% all_miRNAs) %>%
  distinct(miRTarBase.ID, .keep_all = TRUE)
...
#   - dir: Parent directory path
#   - noise_path: Subdirectory for storing nosie comparison plots
...
count_data <- as.data.frame(rowData(sce)) %>% 
  ...
  summarise(n = n()) %>% 
  tidyr::pivot_wider(
    names_from = miRNA_target,
    values_from = n, values_fill = 0
    )
chisq_test <- chisq.test(as.matrix(count_data[, c("Target", "Non-target")]))
p_label_chisq <- ifelse(
  chisq_test$p.value < 0.001,
  "P < 0.001",
...
p8 <- ggplot(
  gene_stats %>%
    filter(gene_class %in% c("HVG", "LVG")),
...
p_label <- ifelse(
  noise_comparison$p.value < 0.001,
  "P < 0.001", 
...
p9 <- ggplot(
  rowData(sce),
  aes(x = miRNA_target, y = Epsilon, fill = miRNA_target)
  ) +
  geom_boxplot(
    width = 0.6,
    outlier.shape = 21,
  ...
  labs(
    x = NULL, y = "Biological Overdispersion",
